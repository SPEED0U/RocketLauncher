...
function runLinuxBuild(version, extraArgs = []) {
  return new Promise((resolve, reject) => {
    ...
    const scriptContent = [
      ...
      'node -e "const fs=require(\'fs\'); const c=JSON.parse(fs.readFileSync(\'src-tauri/tauri.conf.json\',\'utf8\')); delete c.build.beforeBuildCommand; fs.writeFileSync(\'src-tauri/tauri.conf.json\', JSON.stringify(c,null,2));"',
      '',
      '# Build AppImage via Tauri',
      `CARGO_TARGET_DIR=/tmp/rocket-launcher-target npx tauri build --bundles appimage ${extraArgs.join(' ')}`.trimEnd(),
      '',
      ...
      'cp /tmp/rocket-launcher-target/release/bundle/appimage/*.AppImage "$NTFS_BUNDLE/appimage/" 2>/dev/null || true',
